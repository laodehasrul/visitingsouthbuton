<nav x-data="{ isOpen: false }" class="fixed w-full z-50">
            ...
            <div class="flex justify-center lg:flex lg:mt-0 lg:-mx-2">
                <a href="#" class="mx-2 text-white transition-colors duration-300 transform hover:text-gray-500"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20"
                        viewBox="0 0 50 50" fill="none" class="fill-current">
                        <path
    ...
    <div id="navbar" class="navbar relative w-full transition-all duration-500">
        ...
        <div id="navbar" class="relative max-w-6xl px-6 xl:px-0 mx-auto py-4 z-30">
            <div class="lg:flex lg:items-center">
                ...
                <div x-data="{ open: false }" class="w-full hidden lg:block">
                    ...
                    <div x-cloak :class="[isOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full']"
                        ...
                        <div class="absolute w-full h-full bg-white opacity-70 -mx-6 lg:hidden"></div>
                        <div
                            class="flex flex-col text-gray-600 text-sm uppercase font-bold lg:flex lg:px-16 lg:-mx-4 lg:flex-row lg:items-center">
                            <a href="/beranda" 
                                class="mt-2 transition-colors duration-300 transform lg:mt-0 lg:mx-4 hover:text-gray-900 {{ Request::routeIs('beranda') ? 'text-black' : '' }}">Beranda</a>
                            <a href="/event-festival" 
                                class="mt-2 transition-colors duration-300 transform lg:mt-0 lg:mx-4 hover:text-gray-900 {{ Request::routeIs('event-festival') ? 'text-black' : '' }}">Event</a>
                            @foreach ($nav as $item)
                                <div x-data="{ isOpen: false, timeout: null }" x-on:mouseenter="isOpen = true; clearTimeout(timeout)"
                                    x-on:mouseleave="timeout = setTimeout(() => { isOpen = false }, 300)"
                                    class="relative flex items-center mt-2 lg:mt-0 lg:mx-4">
                                    @if (count($item->childrens))
                                    <a class="hover:text-gray-900 transition-colors duration-300 transform {{ request()->is('id/wisata') ? 'text-black' : '' }} || {{ request()->is('id/wisata-pantai') ? 'text-black' : '' }} || {{ request()->is('id/wisata-alam') ? 'text-black' : '' }} || {{ request()->is('id/wisata-buatan') ? 'text-black' : '' }}"
                                        href="{{ route('category', $item->slug) }}" 
                                        class="">{{ $item->title }}</a>
                                    @else
                                    <a class="hover:text-gray-900 transition-colors duration-300 transform {{ route('category', $item->slug) == url()->current() ? 'text-black' : '' }}"
                                        href="{{ route('category', $item->slug) }}" 
                                        class="">{{ $item->title }}</a>
                                    @endif
                                    ...
                                    @if (count($item->childrens))
                                        ...
                                        <ul x-show="isOpen"
                                            class="relative block lg:absolute top-0 mt-12 w-screen bg-white/70 p-4 space-y-1 max-w-max">
                            ...
                            @endforeach
                            <a href="/peta-wisata" 
                                class="mt-2 transition-colors duration-300 transform lg:mt-0 lg:mx-4 hover:text-gray-900 {{ request()->is('peta-wisata') ? 'text-black' : '' }}">Peta
                                Wisata</a>
                        </div>
                        <button x-data x-on:click="$dispatch('open-modal')" type="button"
                            class="text-gray-500 hover:bg-white hover:border-black hover:rounded-full hover:border hover:p-2 hover:text-black transition-all duration-300 ease-in-out">
                        ...
                        <div
                            class="flex items-center w-full lg:w-auto justify-center mt-5 border-solid pt-5 border-t lg:border-0 lg:pt-0 lg:mt-0">
                            <div class="flex items-center space-x-3">
                                @if (Auth::user())
                                    <div class="relative" x-data="{ open: false }" x-on:click.outside="open = false"
                                        x-on:mouseleave="timeout = setTimeout(() => { open = false }, 300)">
                                        <button
                                            class="p-0.5 rounded-full bg-gray-100 hover:bg-teal-500 focus:bg-teal-500"
                                            x-on:click="open = !open">
                                            <img class="rounded-full h-8 w-8" src="{{ auth()->user()->avatarUrl() }}"
                                                alt="">
                                        </button>
