        ...
        <div
            ...
            <form class="ml-auo space-y-4 mb-2">
                <div class="grid grid-cols-2 gap-8 border p-4 rounded-sm">
                    <div>
                        @if ($newimage_story)
                            <img class="object-cover object-center h-[220px] w-full"
                                src="{{ $newimage_story->temporaryUrl() }}" alt="">
                        @else
                            <img class="object-cover object-center h-[220px] w-full"
                                src="{{ asset('storage/story') }}/{{ $story_image }}" alt="">
                        @endif
                        <x-filepond wire:model="newimage_story" allowFileTypeValidation allowImageCrop="true"
                            allowImageEdit acceptedFileTypes="['image/png', 'image/jpg', 'image/JPG', 'image/jpeg']"
                            allowFileSizeValidation maxFileSize="4mb" />
                        @error('newimage_story')
